...
// Replace with your interval, QR code flag, and conditions
unsigned long previousMillis = 0;
const unsigned long interval = 1000; // 1-second interval
bool qrCodeDetected = false; // Set to true when QR code is successfully read
const char *scannedUUID = ""; // Store UUID after QR code is read
/* ======================================== */
  ...
  <body>
    ...
    <script>
      ...
      function send_cmd(cmds) {
        ...
        //xhr.open("POST", "/queues/verify", true);
...
static esp_err_t cmd_handler(httpd_req_t *req){
  ...
  if (resultData == "S") {
    ...
    // Correct ledcAttach syntax
    if (ledcAttach(4, 5000, 8)) { // Attach GPIO pin 4 with 5kHz frequency and 8-bit resolution
        ledcWrite(0, pwm);        // Write PWM value to channel 0
    } else {
    ...
    if (ws_run == true) {
      ...
      if (get_wsLive_interval > 2) {
        ...
        if (wsLive_val == last_wsLive_val) {
          ...
          //Simulate QR code being read
          qrCodeDetected = true;
          scannedUUID = "example-uuid-scanned-from-qr";
        }
      }
    }
  }
